...
Tests (c) test différentiel avec nulle bootstrap et (f) centralité du graphe
COMBINÉ (cocatalyse ∪ PPI ∪ signaling) = à brancher dans une v2 si le (b)/(d)
montre qu'un outil simple est compétitif.
...
-----
    python src/validation/reports/head_to_head_baselines.py \\
        --ranking output/interpretation/V5.4.1/cross_seed_gene_ranking.tsv \\
        --humess-dir data/humess/output_huvec \\
...
def run(ranking: Path, humess_dir: Path, out: Path, cellage: Path,
        with_edges: bool = True, combined: bool = False,
        coexpr_file: Path | None = None, ppi_dir: Path | None = None,
        omni_dir: Path | None = None, betw_k: int = 500) -> pd.DataFrame:
    # 1. canal feature imp_delta
    hm = load_humess_importance(humess_dir)
    hm["abs_imp_delta"] = hm["imp_delta"].abs()
    hm["rank_absdelta"] = _rank_desc(hm["abs_imp_delta"])
    hm["rank_signeddelta"] = _rank_desc(hm["imp_delta"])  # pro-sén = imp_delta > 0

    # 2. canal arêtes cocatalyse (HuMess)
    cocat_edges = None
    if with_edges:
        ...
        if cocat_edges is not None:
            ...
            topo["rank_cocat_diffconn"] = _rank_desc(topo["cocat_diff_conn"].abs())
            hm = hm.merge(topo, on="gene", how="left")

    # 3. colonne vertébrale = ranking GNN (univers complet : inclut les gènes
    #    HORS HuMess comme HMGB2/H2AFZ, nécessaires au test coexpr)
    ...
    merged = gnn[keep].rename(columns={"target": "gene"}).merge(hm, on="gene", how="left")
    merged["rankshift_driver_vs_absdelta"] = merged["rank_driver"] - merged["rank_absdelta"]

    # 4. baselines de graphe externes
    centralities: list[str] = []  # labels ajoutés (cocat compris si présent)
    if "cocat_betweenness" in merged.columns:
        centralities.append("cocat")
    # (f) graphe COMBINÉ cocatalyse ∪ PPI ∪ signaling (toujours sans GNN)
    if combined:
        parts = []
        if cocat_edges is not None:
        ...
        if parts:
            ...
            merged = merged.merge(graph_centrality(comb, "combined", betw_k),
                                  on="gene", how="left")
            centralities.append("combined")
    # baseline COEXPR (réseau GRNBoost2) — analogue WGCNA
    if coexpr_file:
        merged = merged.merge(
    ...
    # 5. métriques : ρ au driver + AUROC vs driver sur support commun
    metrics = {
        "n_humess_genes": int(hm["has_humess"].sum()),
        "rho_absdelta_driver": _rho_driver(merged, "abs_imp_delta")[0],
    }
    ...
    aurocs = {}
    if pos is not None:
        for col, lab in [("abs_imp_delta", "imp_delta")] + \
                        [(f"{l}_betweenness", f"{l}_betw") for l in centralities]:
            if col in merged.columns:
                aurocs[lab] = _auroc_pair(merged, col, pos)

    # 6. sortie TSV
    out = Path(out); out.parent.mkdir(parents=True, exist_ok=True)
    cols = ["gene", "driver_score", "rank_driver", "imp_delta", "abs_imp_delta",
            "rank_absdelta", "rank_signeddelta"]
    for lab in centralities:
    ...
    _print_summary(metrics, aurocs, merged, centralities, out)
    return merged
...
def _focused_table(merged: pd.DataFrame, rank_col: str, targets: list[str],
    ...
    for g, row in show.iterrows():
        if g not in merged["gene"].values:
        ...
        print(f"    {g:8s} : {rb:>14s}  →  driver#{rd}")


def _print_summary(metrics, aurocs, merged, centralities, out) -> None:
    ...
    for lab in centralities:
        print(f"  ρ(betweenness {lab:8s}, driver) = {metrics.get(f'rho_{lab}betw_driver','—')}")
    if aurocs:
    ...
    # feature imp_delta
    print("\n  --- cibles métaboliques §2 : |imp_delta| → GNN ---")
    _focused_table(merged, "rank_absdelta", METAB_TARGETS, "|Δ|")
    # cocatalyse / combiné
    if "cocat" in centralities:
        print("\n  --- §2 : betweenness COCATALYSE seule → GNN ---")
        _focused_table(merged, "rank_cocat_betw", METAB_TARGETS, "cocat-bet")
    if "combined" in centralities:
        print("\n  --- §2 : betweenness COMBINÉ (cocat∪PPI∪signaling) → GNN [test f] ---")
        _focused_table(merged, "rank_combined_betw", METAB_TARGETS, "comb-bet")
    if "coexpr" in centralities:
        print("\n  --- betweenness COEXPR → GNN : cœur chromatine (§1) + métabolique ---")
        _focused_table(merged, "rank_coexpr_betw",
                       CONTRAST_TARGETS + METAB_TARGETS, "coexpr-bet")
    print(f"\n[wrote] {out}")
...
def main():
    ...
    ap.add_argument("--humess-dir", type=Path,
                    ...
                    help="racine HuMess locale (models/<COND>/cs|stats/)")
    ...
    ap.add_argument("--ppi-dir", type=Path, default=ROOT / "data/PPI",
                    help="STRING (9606.protein.links/aliases.v12.0) pour --combined")
    ap.add_argument("--omnipath-dir", type=Path, default=ROOT / "data/omnipath",
                    help="SIGNOR + CollecTRI pour --combined")
    ...
    args = ap.parse_args()
    run(args.ranking, args.humess_dir, args.out, args.cellage,
